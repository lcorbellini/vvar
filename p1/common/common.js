...
var common = (function() {
  ...
  /**
   * @return {?Object} Return the dynamic ad data.
   */
  function getAdData() {
    return document.getElementById(elementId_.gpaDataProvider).getData();
  }
   ...
   * frame-snap-interval Swipegallery attribute. If attribute has not been set
   * defaults to 1.
   * @param {Element} gallery Swipegallery.
  ...
  function getGalleryFrameIndexLast(event) {
    var frameIndexLast =
        getGalleryFrameIndexFirst(event) +
        galleryNumFramesPerPage(event.target) - 1;
